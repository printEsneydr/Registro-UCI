...
class CreateGlasgowForm extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateGlasgowForm> createState() => _CreateGlasgowFormState();
}

class _CreateGlasgowFormState extends ConsumerState<CreateGlasgowForm> {
  int _aperturaOcular = 4;
  int _respuestaVerbal = 5;
  int _respuestaMotora = 6;
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        ElevatedButton(
          onPressed: _guardarGlasgow,
          style: ElevatedButton.styleFrom(
            backgroundColor: Colors.green,
            padding: const EdgeInsets.symmetric(vertical: 16),
          ),
          child: const Text('Guardar Glasgow',
  ...
  void _guardarGlasgow() async {
    try {
      ...
      await ref.read(glasgowControllerProvider.notifier).addGlasgow(
            widget.idIngreso,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Glasgow guardado correctamente')),
        );
      }
    } catch (e) {
      if (mounted) {
